...
class PDDLInterface:
    ...
    def writeProblem(world_info, file="agents/problem.pddl"):
        ...
        with open(file, "w") as f:
            ...
            for resource in resources.keys():
                f.write("(= (resource_size "+str(resource)+") 1)\n ")
            f.write("\n ")
            ...
            #random.shuffle(actorNodes)
            #for act, node in zip(world_info['actors'], actorNodes):
            #    f.write("(alocation a"+str(act)+" node"+str(node)+")\n ")
            #    break
            #for resource, node in zip(resources.keys(), actorNodes):
            #    f.write("(rlocation "+str(resource)+" node"+str(node)+")\n ")
            #    break
            for building, node in zip(world_info['tasks'], buildingNodes):
