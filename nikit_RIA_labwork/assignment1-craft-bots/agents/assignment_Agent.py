...
#from PDDLInterface import PDDLInterface
...
class Assignment_Agent(Agent):
    ...
    def send_action(self, action, params):
        ...
        if action == 'move_actor':
            ...
            print("EVENT: moving!")
        elif action == 'dig_mine':
            self.api.dig_at(params[0], params[3])
            print("EVENT: digging mine!")
        #elif action == 'start_site':
        #    self.api.start_site(params[0], params[1])
        #    print("EVENT: starting site!")
        elif action == 'pick_up_resource':
            self.api.pick_up_resource(params[0], params[2])
            print("EVENT: picking up resource!")
        elif action == 'deposit_resource':
            self.api.deposit_resources(params[0], params[2], params[1])
            print("EVENT: depositing resource!")
        elif action == 'start_construction':
            self.api.construct_at(params[0], params[2])
            print("EVENT: constructing!")
        elif action == 'finish_construction':
            self.api.construct_at(params[0], params[2])
